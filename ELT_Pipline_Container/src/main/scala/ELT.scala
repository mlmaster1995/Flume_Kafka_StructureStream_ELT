...
object ELT extends Serializable {
  ...
  def extract(session: SparkSession = spark): sql.DataFrame =
      session.readStream.format("kafka").option("kafka.bootstrap.servers", s"${brokers(2)},${brokers(3)},${brokers(4)}").option("subscribe", "exec").load()
  ...
  object Load {
    ...
    def to_hdfs(source:sql.DataFrame, format:String, mode:String, compressionType:String, hdfsPath:String, checkpointPath:String ):DataStreamWriter[Row] =
      source
        .writeStream
        .format(format)
        .outputMode(mode)
